...
export default function EditFeature() {
  ...
  let [upfeatures,setUpfeatures]=useState({
    entityName: '',
    description: '',
    featureName: '',
  ...
  let url = "https://featuremarketplacewebapi20231207151555.azurewebsites.net/api/Feature/AddFeature";
  ...
  const handleChange = (evt) => {
    ...
    setUpfeatures({
      ...upfeatures,
      [id]: value,
      featureDataType: id === "featureDataType" ? value : upfeatures.featureDataType
    });
  };
  ...
  const validateForm = () => {
    ...
    const validations = {
      ...
      value: {
        condition: value => {
          switch (upfeatures.featureDataType) {
            case "int":
              return /^\d+$/.test(value);
            case "float":
    ...
    for (const [field, validation] of Object.entries(validations)) {
      if (!validation.condition(upfeatures[field])) {
  ...
  return (
    ...
    <div className='h4 m-4' >Edit Feature</div>
